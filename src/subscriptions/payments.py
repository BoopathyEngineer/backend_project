    ...
    def coupon_validation(event: Dict, user: str, email: str, username: str) -> Dict:
        ...
        if queryparams:
            ...
            if items:
                ...
                # if isinstance(items,list) an d len(items) == 0:  ### Not Support to this plan
                ...
                if isinstance(items,list) and len(items) > 0:
                    ...
                    #New 
                    if api_id in ['0','1','2','3','4','5']:
                        pass
                    elif api_id.split('.')[1] in ['1', '2','3']:
                        if len(plan_name)>1:
                            if coupon_type == 'product':
                                prod_plan = plan_name[0]
                                prod_specific = querysort(coupontable,'coupon_code',coupon_code,'api_id',prod_plan)
                            elif coupon_type == 'addon':
                                addon_plan = plan_name[1]
                                prod_specific = querysort(coupontable,'coupon_code',coupon_code,'api_id',addon_plan)
                        else:
                            plan = plan_name[0]
                            prod_specific = querysort(coupontable,'coupon_code',coupon_code,'api_id',plan)
                        if prod_specific is None or prod_specific == []:
                            response={'success':False,'message':'Coupon not applicable to selected items.','statusCode':200,'id':2}
                            return response     
                    ...
                    # New 
                        ...
                        res = {'success': True, 'message': 'product and addon', 'coupon_type': coupon_type,'discount_type': discount_type, 'discount_value': discount_value,'min_purchase': min_purchase, 'statusCode': 200}
                    ...
                    response={'success':False,'message':'Coupon not applicable to selected items.','statusCode':200,'id':0}
                    return response
            else:
                response={'success': False, 'message': 'An error occured', 'statusCode': 400,'id':0}
                return response
            ...
            if items:
                if res['success'] :
                    ...
                    elif (coupon_type == 'both' and len(plan_name) == 1):
                        a = plan_name[0]
                        current_plan = a.split('.')
                        plan = current_plan[1]
                        if plan == '1':  #case-product
                            addon_initial = 0
                            prod_initial_amount=int(prod_details.get('price'))
                            initial=prod_initial_amount
                            i=initial
                            coupon_type_id=0
                        elif plan == '3': #case-addon
                            initial=0
                            addon_initial_amount =int(addon_initial_amount)
                            addon_stripe_id=addon_details.get('stripe_id')
                            for price, count, plan_id in zip(addon_stripe_id, quantity, plan_name):
