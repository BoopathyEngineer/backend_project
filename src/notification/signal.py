...
def save_notification_to_db(signal, **named):
    ...
    FilterExpression=Attr('message').eq(verb) & Attr('timestamp').begins_with(today_date)
    duplicate_check = query_item(table,key='email',value=recipient,expression=FilterExpression)
